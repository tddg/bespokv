...
int RedisLink::convert_req(){
	...
	if(it == cmd_table.end()){
		...
		for(unsigned i=1; i<recv_bytes.size(); i++){
	...
	if(this->req_desc->strategy == STRATEGY_ZADD){
		...
		if(recv_bytes.size() >= 2){
			recv_string.push_back(recv_bytes[1].String());
			for(unsigned i=2; i<=recv_bytes.size()-2; i+=2){
	...
	if(this->req_desc->strategy == STRATEGY_ZRANGEBYSCORE || this->req_desc->strategy == STRATEGY_ZREVRANGEBYSCORE){
		...
		if(recv_bytes.size() >= 4){
			...
			for(unsigned i=4; i<recv_bytes.size(); i++){
...
#ifdef USE_SMART_PTR
const std::vector<Bytes>* RedisLink::recv_req(std::shared_ptr<Buffer> input){
#else
const std::vector<Bytes>* RedisLink::recv_req(Buffer *input){
#endif
	int old_sz = input->size();
	int ret = this->parse_req(input);
	if(ret == -1){
		return NULL;
	}
	// yue: record last payload size for data ptr reset right before send
	input->remember_sz(old_sz);
	//LOG(INFO) << "old_sz=" << old_sz << ", new_sz=" << input->size();
	...
	// Bytes don't hold memory, so we first copy Bytes into string and store in a
	// vector of string, then create Bytes-es pointing to strings
	recv_bytes.clear();
	//LOG(INFO) << "recv_string.size=" << recv_string.size();
	for(unsigned i=0; i<recv_string.size(); i++){
...
int RedisLink::send_resp(Buffer *output, const std::vector<std::string> &resp){
	...
	if(req_desc->reply_type == REPLY_MULTI_BULK){
		...
		for(unsigned i=1; i<resp.size(); i++){
...
#ifdef USE_SMART_PTR
int RedisLink::parse_req(std::shared_ptr<Buffer> input){
#else
