...
void Fdevents::set_proxy(NetworkServer *proxy) {
	this->proxy = proxy;
}

long unsigned Fdevents::get_events_size(){
...
int Fdevents::clr(int fd, int flags){
	...
	if(fde->s_flags & FDEVENT_IN)  in_ev++;
...
/*
 * yue: event_wait does not batch events, instead triggers events
 * one by one
 */
int Fdevents::event_wait(int timeout_ms) {
	struct Fdevent *fde;
	struct epoll_event *epe;
	...
	for ( ;; ) {
		...
		if (nfds == 0) {
			if (timeout_ms == -1) {
				fprintf(stderr, "epoll wait on e %d with %d events and %d timeout "
						"returned no events", ep_fd, MAX_FDS, timeout_ms);
				return -1;
			}
		...
		for(int i = 0; i < nfds; i++) {
			...
			proxy->event_process(fde);

			return nfds;
		}

		fprintf(stderr, "epoll wait on e %d with %d events failed: %s", ep_fd, 
