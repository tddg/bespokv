...
#elif defined(USE_LOG)
	hashmap = new std::unordered_map<std::string, struct stored>();
	CHECK(hashmap != NULL);
	FILE *fptr = fopen("iGarbage.db", "wb");
	if (!fptr) {
		fprintf(stderr, "failed to open iLog.log\n");
		exit(EXIT_FAILURE);
	}
	db = malloc(sizeof(struct iLog));
	CHECK(db != NULL);
	db->wlog = fptr;
	db->rlog = NULL;
	db->end = 0;
...
static void handle_request(Link *c) {
	...
	if (get.GetTypeName() == t) {  // yue: serve GET
		...
		if (hashmap->find(key) != hashmap->end()) {
			...
			size_t pos = stored.pos;
			size_t len = stored.len;
			//fprintf(stderr, "pos=%zu len=%zu\n", pos, len);
			char val[256];
			if (db->rlog == NULL) {
				db->rlog = fopen("iGarbage.db", "rb");
			}
			fseek(db->rlog, pos, SEEK_SET);
			fread(val, len, 1, db->rlog);
			fseek(db->rlog, 0, SEEK_END);
			std::string value(val, len);
			//fprintf(stderr, "fetched val=%s\n", val);
			reply.set_status(0);
			reply.set_value(value);
		} else {
			reply.set_status(-1);
		...
		auto item = hashmap->find(req.mutable_put()->key());
		if (item != hashmap->end()) { // yue: if kv pair exists
			size_t pos = ftell(db->wlog); 
			item->second.pos = pos;
			std::string val = req.mutable_put()->value();
			item->second.len = val.size();
			//fprintf(stderr, "pos=%zu len=%zu\n", pos, val.size());
			fwrite(val.c_str(), val.size(), 1, db->wlog);
		} else { // yue: if kv pair does not exist
			size_t pos = ftell(db->wlog); 
			std::string val = req.mutable_put()->value();
			...
			//fprintf(stderr, "pos=%zu len=%zu\n", pos, val.size());
			fwrite(val.c_str(), val.size(), 1, db->wlog);
			hashmap->insert(std::make_pair(req.mutable_put()->key(), stored));
		}
		reply.set_status(0);
#endif
...
int main(int argc, char **argv) {
	...
	db_init(settings.hashpower_init);
