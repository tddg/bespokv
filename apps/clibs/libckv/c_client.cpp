...
/**
 * yue: replica-aware init interface
 */
void client_init_replica(void *client, char ***hosts, uint32_t **ports, 
		size_t num_hosts, size_t num_replicas, 
		int get_from_which_replica, int set_to_which_replica,
		const int failover) {
  douban::mc::Client* c = static_cast<Client*>(client);
  c->init_replica(hosts, ports, num_hosts, num_replicas, get_from_which_replica, set_to_which_replica);
  if (failover) {
    c->enableConsistentFailover();
  } else {  // yue: so far always comes here; by default failover is disabled
    c->disableConsistentFailover();
  }
}
