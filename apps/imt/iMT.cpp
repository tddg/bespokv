...
static void handle_request(Link *c) {
	...
	if (get.GetTypeName() == t) {  // yue: serve GET
		...
		if (hashmap->find(key) != hashmap->end()) {
			...
			char val[256];
			fseek(db->log, pos, SEEK_SET);
			fread(val, len, 1, db->log);
			fseek(db->log, 0, SEEK_END);
